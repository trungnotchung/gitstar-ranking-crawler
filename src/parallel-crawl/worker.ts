...
const currentProxyUrl = proxyUrls[workerId as keyof typeof proxyUrls] || PROXY_URL_1;
...
repoQueue.process(async (job) => {
  ...
  console.log(`🚀 Worker ${workerId} (Proxy: ${currentProxyUrl}) Processing repo: ${repoFullName}`);
  ...
  try {
    ...
    const result = await getAllReleasesAndCommits(repoFullName, currentProxyUrl);
    ...
    console.error(`❌ Worker ${workerId} error processing ${repoFullName}:`, error.message);
    throw error; 
  }
});
